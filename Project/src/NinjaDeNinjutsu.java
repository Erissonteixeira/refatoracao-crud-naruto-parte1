...
public abstract class NinjaDeNinjutsu extends Personagem implements Ninja {
    ...
    public void usarJutsu(Ninja inimigo) {
        ...
        if (!temChakra(jutsu.getConsumoDeChakra())) {
            System.out.println(nome + " Está sem chakra para usar Ninjutsu!");
            return;
        }

        gastarChakra(jutsu.getConsumoDeChakra());
        System.out.println(nome + " Usou Ninjutsu causando " + jutsu.getDano() + " de dano!");
        inimigo.desviar(jutsu.getDano());
    }

    @Override
    public void desviar(int dano) {
        boolean conseguiuDesviar = new Random().nextInt(100) < 40;

        if (conseguiuDesviar) {
            System.out.println(nome + " Desviou do Ninjutsu!");
        } else {
            receberDano(dano);
